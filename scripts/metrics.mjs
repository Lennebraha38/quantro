...
async function main() {
  ...
  if (!res.ok || !j) {
    console.error(`✗ PSI hatası ${res.status}: ${(j && j.error && j.error.message) || 'yanıt yok'}`);
    console.error('İpucu: PSI_API_KEY ortam değişkenini ayarlayın (Google Cloud → PageSpeed Insights API).');
    process.exit(1);
  }
  ...
  if (Object.keys(fe).length) {
    ...
    for (const [name, cfg] of Object.entries(FIELD)) {
      ...
      const cat = name === 'EXPERIENCE' ? String(m.category || '') : (p99 <= cfg.good ? 'GOOD' : (p99 <= cfg.poor ? 'NEEDS_IMPROVEMENT' : 'POOR'));
  ...
  const score = lh.categories && lh.categories.performance && Math.round(lh.categories.performance.score * 100);
  console.log(`  Performans: ${score == null ? '—' : score + '/100'}`);
  const audits = lh.audits || {};
  for (const a of ['first-contentful-paint', 'largest-contentful-paint', 'cumulative-layout-shift', 'speed-index', 'total-blocking-time', 'interactive']) {
    const x = audits[a];
    if (x && x.displayValue) console.log(`  ${a.split('-').map(w => w.toUpperCase()).join(' ').padEnd(26)} ${x.displayValue}`);
  }
