...
if (!m) {
  ...
  console.error(out.split('\n').filter((l) => /^# (tests|pass|fail|ok|not ok)/.test(l)).join('\n'));
  process.exit(res.status === 0 ? 1 : res.status || 1);
}

const [line, branch, funcs] = m.slice(1).map(Number);
const rows = { line, branch, funcs };
console.log(`▸ Kapsam: satır %${line} · dal %${branch} · fonksiyon %${funcs} (eşikler: satır ${THRESHOLDS.line}% · dal ${THRESHOLDS.branch}% · fonksiyon ${THRESHOLDS.funcs}%)`);
