...
function checkVerelConfig() {
  ...
  try {
    ...
    if (!Array.isArray(cfg.headers) || !Array.isArray(cfg.redirects) || !Array.isArray(cfg.rewrites)) {
      throw new Error('beklenen alanlar eksik (headers/redirects/rewrites)');
    }
    console.log(`  ✓ vercel.json geçerli (${cfg.headers.length} header, ${cfg.redirects.length} redirect, ${cfg.rewrites.length} rewrite)`);
  } catch (e) {
...
function checkHtmlFile(file) {
  ...
  if (!/<!doctype html>/i.test(raw)) errors.push(`${path.relative(root, file)}: <!DOCTYPE html> bulunamadı`);
  ...
  while ((m = re.exec(raw)) !== null) {
    ...
    if (!fs.existsSync(target)) errors.push(`${path.relative(root, file)} → kırık referans: ${url}`);
  }
}
...
for (const f of htmlFiles) totals.html++;
...
console.log(`  ✓ içerik: ${totals.html} HTML · ${totals.js} JS · ${totals.css} CSS · ${totals.sql} SQL`);
