...
async function withFetch(impl, fn) {
  ...
  global.fetch = (url, init) => {
    ...
    return impl ? impl(url, init) : Promise.resolve({ ok: true, status: 200, json: async () => [] });
  };
  try { return await fn(); }
...
test('comments — halka açık liste (slug) supabase URL ile çağrılır', async () => {
  ...
  const res = mockRes();
  await withFetch(() => Promise.resolve({ ok: true, status: 200, json: async () => rows }), () =>
...
test('comments — geçerli yorum eklenir (is_approved=false)', async () => {
  const res = mockRes();
  let posted;
  await withFetch(() => Promise.resolve({ ok: true, status: 201, json: async () => [] }), () =>
    comments(makeReq('POST', { body: { post_slug: 'x', name: 'N', content: 'abcd', email: 'a@b.co' }, ip: 'H' }), res)
  ).then(() => { posted = calls[0].init.body; });
...
test('comments — admin yönetim dalı geçersiz token ile 401', async () => {
  const res = mockRes();
  await withFetch(null, () =>
    comments(makeReq('GET', { bearer: 'bos.deger', ip: 'I' }), res)
  );
  assert.equal(res.statusCode, 401);
});
...
test('blog — geçerli POST 201 ile yeni kaydı döner', async () => {
  ...
  const res = mockRes();
  const token = signToken('admin');
  await withFetch(() => Promise.resolve({ ok: true, status: 201, json: async () => created }), () =>
    blog(makeReq('POST', { body: { title: 'Yeni', content: 'içerik', published: 1 }, bearer: token, ip: 'M' }), res)
  );
  assert.equal(res.statusCode, 201);
