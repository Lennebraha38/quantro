...
for (const [slug, langs] of Object.entries(groups)) {
  ...
  try {
    ...
    console.error(`blog_posts ${slug}: ${e.message}`);
  }
  for (const [lang, p] of Object.entries(langs)) {
    try {
      ...
      console.error(`blog_posts_i18n ${slug} ${lang}: ${e.message}`);
    }
  }
}

console.log(
  `▸ Import tamam: ${posts} ana kayıt (blog_posts) · ${i18nRows} çeviri satırı (blog_posts_i18n)`,
);
