...
function rend() {
  for (let q = 0; q < 2; q++) {
    document.getElementById("l" + q).innerHTML = circ[q]
      .map((g) => `<div class="gel ${g.g}" onclick="rmG('${g.g}',${q})">${g.g}</div>`)
      .join("");
    if (q === 1 && circ[0].some((g) => g.g === "CNOT"))
      document.getElementById("l1").innerHTML +=
        `<div class="gel CNOT" style="border-style:dashed">⊕</div>`;
  }
  document.getElementById("cr").style.display = "none";
}
function rmG(g, q) {
  const i = circ[q].findIndex((x) => x.g === g);
  if (i > -1) circ[q].splice(i, 1);
  rend();
}
function rc() {
  circ = [[], []];
...
async function mc() {
  ...
  document.getElementById("crb").innerHTML = states
    .map((l, i) => {
  ...
  if (ex)
    ex.innerHTML = `${t("t2.exact")}: ${states
      .map(
        (l, i) =>
