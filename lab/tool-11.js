...
function tunCalc() {
  ...
  document.getElementById("tun-tv").textContent =
    T > 1e-4 ? (T * 100).toFixed(T > 0.1 ? 1 : 3) + " %" : T.toExponential(1);
  document.getElementById("tun-kv").textContent =
    kappa !== undefined ? kappa.toFixed(2) + " nm⁻¹" : "—";
...
function tunLoop(c) {
  ...
  const bl = w * 0.42,
    br = bl + Math.max(18, Math.min(90, a * 140));
  ...
  ctx.stroke();
  /* barrier */
  ctx.fillStyle = "rgba(120,90,255,.22)";
  ctx.fillRect(bl, h - 30 - wallV, br - bl, wallV);
  ctx.strokeStyle = "rgba(160,130,255,.8)";
  ctx.lineWidth = 1.5;
  ctx.strokeRect(bl, h - 30 - wallV, br - bl, wallV);
  ctx.font = "11px monospace";
  ctx.fillStyle = "rgba(160,130,255,.9)";
  ctx.textAlign = "center";
  ctx.fillText(
    t("t11.canvas.barrier").replace("{0}", V.toFixed(1)),
    (bl + br) / 2,
    h - 30 - wallV - 8,
  );
  /* energy line */
  ctx.strokeStyle = "rgba(0,200,240,.6)";
  ctx.setLineDash([5, 4]);
  ctx.beginPath();
  ...
  if (tunPkt.decided) {
    if (pass) {
      ...
      ctx.fillText(t("t11.canvas.pass"), 10, 16);
    } else {
      ctx.fillStyle = "rgba(255,150,60,.9)";
      ctx.fillText(t("t11.canvas.reflect"), 10, 16);
    }
  } else {
    ctx.fillStyle = "rgba(255,255,255,.35)";
    ctx.fillText(t("t11.canvas.super").replace("{0}", (T * 100).toFixed(1)), 10, 16);
  }
  ...
  if (tunPkt.ref && tunPkt.ref.p > -60) {
    tunPkt.ref.p -= sp;
  }
  if (tunPkt.p > w + 80 && (!tunPkt.ref || tunPkt.ref.p < -60)) {
    tunPkt.p = -80;
    tunPkt.ref = null;
    tunPkt.decided = false;
  }
