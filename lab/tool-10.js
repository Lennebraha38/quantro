/* ══ T10: SCHRODINGER CAT ══ */
let catP=0.5,catState='super',catRaf=null,catT=0,catLog=[];
function catSetP(){catP=+document.getElementById('cat-ps').value/100;document.getElementById('cat-pv').textContent=Math.round(catP*100)+'%';catT=0;catDraw()}
function catStart(){if(!document.getElementById('cat-canvas')._inited){document.getElementById('cat-canvas')._inited=true;catDraw()}}
function catReset(){
...
function catDraw(){
  ...
  const W=c.width=Math.max(280,c.clientWidth)*window.devicePixelRatio,dpr=window.devicePixelRatio||1;
  ...
  catT+=0.03;
  const t=catT;
  const bx=w*0.18,by=h*0.12,bw=w*0.64,bh=h*0.62;
  ...
  /* lid */
  roundRect(ctx,bx,by-6,bw,10,4);ctx.fillStyle='#0d1f35';ctx.fill();ctx.stroke();
  ...
  ctx.globalAlpha=1;
  ctx.fillStyle='#0d1f35';ctx.strokeStyle='#00c8f0';
  ctx.beginPath();ctx.arc(ax,ay,16,0,7);ctx.fill();ctx.stroke();
  ctx.font='10px monospace';ctx.textAlign='center';ctx.fillStyle='#00c8f0';
  ctx.fillText(catState==='super'?'?':'✓',ax,ay+4);

  /* flask */
  const fx=ix+iw/2, fy=iy+ih-40;
  ctx.fillStyle='rgba(120,255,160,.15)';
  ctx.strokeStyle='rgba(120,255,160,.5)';
  ctx.beginPath();
  ctx.moveTo(fx-12,fy-58);ctx.lineTo(fx+12,fy-58);
  ...
  ctx.beginPath();ctx.ellipse(fx,fy-8,9,4,0,0,7);ctx.fill();

  /* cat */
  ctx.textAlign='center';ctx.textBaseline='middle';
  const cxm=ix+iw-78,cym=iy+ih-44;
  if(catState==='super'){
    ctx.save();
    ctx.filter='blur(2px)';
    ctx.globalAlpha=.55;
    ...
    ctx.restore();
    ctx.globalAlpha=1;
    ctx.font='bold 30px monospace';ctx.fillStyle='rgba(0,200,240,.8)';
    ctx.fillText('|ψ⟩ = ?',ix+iw/2,iy+30);
  }else{
    ctx.font='68px serif';
    if(catState==='alive'){ctx.fillText('🐱',cxm,cym)}
    else{
