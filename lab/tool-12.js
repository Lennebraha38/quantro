/* ══ T12: DOUBLE SLIT ══ */
var ds={cv:null,cx:null,W:900,H:360,srcX:70,barX:310,scale:0.0038,NB:700,y0:44,y1:316,
       shots:0,total:200,which:false,a:0.10,sep:0.45,lam:0.06,cd:160,
       bins:null,raf:null,flying:false,t0:0,fx:0,fy:0,tx:0,ty:0,spawnSlit:0};
function dsGet(id){return document.getElementById(id)}
function dsStart(){
  ds.cv=dsGet('ds-canvas');
  if(!ds.cv)return;
  if(ds.cv.width!==ds.W){ds.cv.width=ds.W;ds.cv.height=ds.H}
...
function dsRead(){
  const s=dsGet('ds-slit'),d=dsGet('ds-sep'),w=dsGet('ds-wave'),n=dsGet('ds-n');
  if(!s||!d||!w||!n)return;
  ds.a=+s.value/100;
  ds.sep=+d.value/100;
  ds.lam=+w.value/100;
...
function dsSample(cdf){
  ...
  while(loI<hiI){
    const mid=(loI+hiI)>>1;
    if(cdf[mid+1]<r)loI=mid+1;else hiI=mid;
  }
...
function dsLaunchNext(){
  if(ds.shots>=ds.total){ds.flying=false;return}
  ds.shots++;
...
function dsTick(now){
  ...
  if(ds.flying){
    ...
    ds.cx.fill();
    if(p>=1){
      const bi=Math.max(0,Math.min(ds.NB-1,Math.round((ds.ty-ds.y0)/((ds.y1-ds.y0)/ds.NB))));
      ds.bins[bi]++;
...
function dsDrawBase(){
  ...
  const aPx=ds.a*260,sepPx=ds.sep*150,cy=H/2;
  const s1=cy-sepPx/2,s2=cy+sepPx/2;
...
function dsDrawHist(){
  const ctx=ds.cx,W=ds.W,scx=ds.W-110;
  let max=1;
