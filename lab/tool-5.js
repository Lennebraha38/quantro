...
function drawHeis(dx, dp) {
  ...
  const sigma = dx * 10;
  ctx.beginPath();
  ctx.strokeStyle = "rgba(0,200,240,0.8)";
  ctx.lineWidth = 1.5;
  for (let x = 0; x <= W; x++) {
    const xv = (x - W / 2) / 10;
  ...
  const sigmaP = 0.5 / dx;
  ctx.beginPath();
  ctx.strokeStyle = "rgba(160,96,255,0.7)";
  ctx.lineWidth = 1.5;
  for (let x = 0; x <= W; x++) {
    const xv = (x - W / 2) / 10;
  ...
  ctx.fillText(t("t5.canvas.pos"), 8, 16);
  ctx.fillStyle = "rgba(160,96,255,.6)";
  ctx.fillText(t("t5.canvas.mom"), 8, H / 2 + 24);
}
...
function initHeis3DInner() {
  ...
  scene.add(
    new THREE.Line(
      geo,
      new THREE.LineBasicMaterial({ color: 0x00c8f0, opacity: 0.8, transparent: true }),
    ),
  );
  // axes
  [
  ...
  const h = {
    ...
    tm: (e) => {
      if (!drag) return;
      rotY += (e.touches[0].clientX - ox) * 0.01;
      rotX += (e.touches[0].clientY - oy) * 0.01;
      ox = e.touches[0].clientX;
      oy = e.touches[0].clientY;
    },
  };
  ...
  h.vis = () => {
    if (!document.hidden)
      setTimeout(() => {
        if (window.heis3dLost)
          try {
            initHeis3D();
          } catch (e) {}
        else if (window.heis3d) {
          try {
            window.heis3d.r.render(window.heis3d.scene, window.heis3d.cam);
          } catch (e) {}
        }
      }, 30);
  };
  ...
  function anim() {
    ...
    scene.rotation.y += 0.005 + rotY * 0.05;
...
function heisWatch() {
  ...
  if (!canvas) return;
  if (!window.THREE) {
    window.heisFails = (window.heisFails || 0) + 1;
    if (window.heisFails > 2) heisFail(true);
    return;
  }
  const S = window.heis3d;
  if (!S) {
...
if (!window.heis3dWatch) {
  window.heis3dWatch = setInterval(heisWatch, 2500);
}
