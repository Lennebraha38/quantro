...
function rstBB84() {
  ["ab", "aba", "bb", "br"].forEach((id) => (document.getElementById(id).innerHTML = ""));
  ["ak", "bk"].forEach((id) => (document.getElementById(id).textContent = "—"));
...
async function rBB84() {
  ...
  let qbits = null;
  try {
    qbits = await Qrng.bytes(16);
  } catch (e) {
    qbits = null;
  }
  const qb = (i) => (qbits ? qbits[i] & 1 : Math.round(Math.random()));
  const qbase = (i) => (qbits ? (qbits[i] >> 1) & 1 : Math.random() > 0.5 ? 1 : 0);
  ...
  const ak = mi.map((i) => aB[i]).join("");
  const bk = mi.map((i) => bRes[i]).join("");
  let errs = 0;
  mi.forEach((i) => {
    if (aB[i] !== bRes[i]) errs++;
  });
  ...
  document.getElementById("bb").innerHTML = bBas
    .map((b, i) => `<div class="qbit ${aBas[i] === b ? "match" : "mismatch"}">${b}</div>`)
    .join("");
  document.getElementById("br").innerHTML = bRes
    .map((b, i) => `<div class="qbit ${mi.includes(i) ? "match" : ""}">${b}</div>`)
    .join("");
  ...
  setTimeout(() => {
    ...
    sp.textContent = sec + "%";
    sp.style.color = sec > 70 ? "var(--green)" : sec > 40 ? "var(--cyan)" : "var(--red)";
  }, 100);
  qlog(t("t4.log.start"));
