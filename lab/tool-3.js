...
function uU() {
  ...
  for (let i = 1; i <= 8000; i++) {
    const a = i / 8000;
    const E2 = Or / (a * a) + Om / a + Ol * a * a;
    if (E2 > 0) age += 1 / (Math.sqrt(E2) * 8000);
  }
  age /= H0g;
  const tot = Om + Ol + Or;
  const geo =
    Math.abs(tot - 1) < 0.05 ? t("t3.geo.flat") : tot > 1 ? t("t3.geo.closed") : t("t3.geo.open");
  const omk = 1 - tot;
  const q0 = Or + Om / 2 - Ol;
...
function rU() {
  document.getElementById("h0").value = 67.4;
  document.getElementById("om").value = 0.315;
  document.getElementById("ol").value = 0.685;
  document.getElementById("orr").value = 0.00009;
  uU();
}
uU();
