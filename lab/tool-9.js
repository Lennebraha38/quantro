let bloch = null;
function thv() {
  return +document.getElementById("bloch-th").value;
}
function phv() {
  return +document.getElementById("bloch-ph").value;
}
...
function blochReadout() {
  const th = (bloch.th * Math.PI) / 180,
    ph = (bloch.ph * Math.PI) / 180;
...
class BlochSphere {
  ...
  rot(p) {
    const [x, y, z] = p;
    const c1 = Math.cos(this.yaw),
      s1 = Math.sin(this.yaw);
    const x1 = c1 * x + s1 * z,
      z1 = -s1 * x + c1 * z;
    const c2 = Math.cos(this.pitch),
      s2 = Math.sin(this.pitch);
    const y2 = c2 * y - s2 * z1,
      z2 = s2 * y + c2 * z1;
    return [x1, y2, z2];
  }
  s2(p) {
    const [x, y] = this.rot(p);
    return [this.cx + this.R * x, this.cy - this.R * y];
  }
  draw() {
    const c = this.c,
      ctx = this.ctx;
    ...
    ctx.fillStyle = "rgba(2,8,16,.65)";
    ctx.fill();
    for (let m = 0; m < 6; m++) {
      const pts = [];
      for (let i = 0; i <= 36; i++) pts.push(this.vec(i * 5, m * 30));
      arc(pts, "rgba(0,200,240,.16)");
    }
    for (let m = 1; m < 6; m++) {
      const pts = [];
      for (let i = 0; i <= 36; i++) pts.push(this.vec(m * 30, i * 10));
      arc(pts, "rgba(0,200,240,.13)");
    }
    arc(
      [...Array(37)].map((_, i) => this.vec(90, i * 10)),
      "rgba(0,200,240,.35)",
      1.3,
    );
    ...
    ctx.strokeStyle = "rgba(0,200,240,.28)";
    ctx.setLineDash([4, 4]);
    ctx.beginPath();
    ctx.moveTo(ex, ey);
    ctx.lineTo(px, py);
    ...
    ctx.lineTo(ex + ah * 0.6 * Math.cos(ang + 2.5), ey + ah * 0.6 * Math.sin(ang + 2.5));
    ctx.lineTo(ex + ah * 0.6 * Math.cos(ang - 2.5), ey + ah * 0.6 * Math.sin(ang - 2.5));
    ctx.closePath();
    ctx.fill();
    ctx.strokeStyle = "rgba(0,200,240,.5)";
    ctx.lineWidth = 4;
    ctx.beginPath();
  ...
  axis(v, label, rgb) {
    ...
    ctx.strokeStyle = "rgba(232,237,245,.4)";
    ctx.lineWidth = 1;
    ctx.beginPath();
